...
@MainActor final class DraftMovieServiceTests: XCTestCase {

    private var sut: DraftMovieService { DraftMovieService.shared }

    func test_single() {
        let item = sut.single(query: "My Custom Movie")
        XCTAssertEqual(item.title, "My Custom Movie")
        XCTAssertNil(item.sourceId)
        XCTAssertEqual(item.sourceName, DraftMovieService.serviceName)
    ...
    func test_search() async throws {
        ...
        XCTAssertEqual(results.first?.sourceName, DraftMovieService.serviceName)
    ...
    func test_getSourceUrl_encodesTitle() throws {
        let item = ExternalMovieItem(title: "Hello World", sourceName: DraftMovieService.serviceName, sourceId: nil)
